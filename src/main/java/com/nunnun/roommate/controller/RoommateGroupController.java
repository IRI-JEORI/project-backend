...
public class RoommateGroupController {
    ...
    @GetMapping("/{id}") public ResponseEntity<ApiResponse<RoommateGroupDetailResponse>> getDetail(@AuthenticationPrincipal AuthenticatedUser user, @PathVariable Long id) { return ResponseEntity.ok(ApiResponse.success(service.getDetail(user.userId(), id))); }
