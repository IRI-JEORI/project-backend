...
class SleepControllerTest {
    ...
    void createsSleepSessionForAuthenticatedUserUsingServerTime() throws Exception {
        ...
        mockMvc.perform(post("/me/sleep")
                        ...
                        .content("{\"userId\":999,\"sleepDate\":\"2020-01-01\",\"startedAt\":\"2020-01-01T00:00:00\"}"))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.sleepDate").value("2026-08-12"))
                .andExpect(jsonPath("$.data.startedAt").value("2026-08-12T23:40:00"))
    ...
    void rejectsDuplicateFeedbackWithoutChangingExistingScore() throws Exception {
        ...
        sleepFeedbackRepository.saveAndFlush(SleepFeedback.create(user, TODAY, SleepScore.GOOD));
        ...
        mockMvc.perform(post("/me/sleep-feedback")
                ...
                .andExpect(jsonPath("$.error.code").value("SLEEP_FEEDBACK_ALREADY_EXISTS"));

        assertThat(sleepFeedbackRepository.findAll()).singleElement()
    ...
    private org.springframework.test.web.servlet.ResultActions postSleep(User user) throws Exception {
        return mockMvc.perform(post("/me/sleep").header("Authorization", bearerTokenFor(user)));
    }
    ...
    static class FixedClockConfiguration {
        ...
        @Primary
        Clock fixedClock() {
            return Clock.fixed(Instant.parse("2026-08-12T14:40:00Z"), ZoneId.of("Asia/Seoul"));
        }
    }
}
